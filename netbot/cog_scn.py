...
from redmine.redmine import BLOCKED_TEAM_NAME, Client
...
class NewUserModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if user is None:
            log.error(
                f"Unable to create user for {self.login}, {first} {last}, {email}"
            )
            await interaction.response.send_message(
                f"Unable to create user for {self.login}"
            )
            return

        # create the mapping so it the discord user can be found
        self.redmine.user_mgr.create_discord_mapping(
...
class ApproveButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if user:
            ...
            await interaction.response.send_message(
                f"Approved registered user: @{name} {user.login} {user.name}"
            )
        else:
...
class ApproveUserView(discord.ui.View):
    ...
    async def button_callback(
        ...
        await interaction.response.send_message(
            f"ApproveUserView: {button} {interaction}"
        )
...
class IntakeView(discord.ui.View):
    ...
    async def select_callback(
        ...
        await interaction.response.send_message(
            f"IntakeView.select_callback() selected: {select.values[0]}"
        )

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            f"IntakeView.allback() {interaction.data}"
        )
...
class SCNCog(commands.Cog):
    ...
    @option(
        ...
        autocomplete=basic_autocomplete(default_ticket),
    )
    @option(
        "member", description="Discord member collaborating with ticket", optional=True
    )
    async def add(
        self,
        ...
        if member:
            log.info(
                f"Overriding current user={ctx.user.name} with member={member.name}"
            )
            discord_id = member
        ...
        if user:
            ...
            if id_from_user and id_from_user.id > 0:
                # a valid
                await ctx.respond(
                    f"Discord user: {discord_id} is fully configured as redmine user: {user.login}"
                )
            else:
                # need to update
                self.redmine.user_mgr.create_discord_mapping(
                    user, discord_id.id, discord_id.name
                )
                await ctx.respond(
                    f"Discord user: {discord_id.id},{discord_id.name} has been paired with redmine user: {redmine_login}"
                )
        else:
            ...
            if user and self.is_admin(ctx.user):
                ...
                await ctx.respond(
                    f"Discord user: {discord_id.name} has been paired with redmine user: {redmine_login}"
                )
            else:
                # case: unknown redmine_login -> new user request: register new user
                modal = NewUserModal(
                    self.redmine, redmine_login, title="Register new user"
                )
    ...
    async def sync(self, ctx: discord.ApplicationContext):
        ...
        if isinstance(ctx.channel, discord.Thread):
            ...
            if ticket:
                await ctx.respond(
                    f"SYNC ticket {ticket.id} to thread: {thread.name} complete"
                )
            else:
                ...
                if ticket_id:
                    await ctx.respond(
                        f"No ticket (#{ticket_id}) found for thread named: {thread.name}"
                    )
                else:
                    # create new ticket
                    subject = thread.name
                    ...
                    tracker = self.bot.redmine.ticket_mgr.get_tracker(
                        thread.parent.name
                    )
                    if tracker:
    ...
    def find_role(
        self, ctx: discord.ApplicationContext, rolename: str
    ) -> discord.Role | None:
            ...
            if team:
                ...
                all_teams = ", ".join(
                    [
                        ...
                        if team.name.endswith("-team")
                    ]
                )
                await ctx.respond(
                    f"Unknown team name: {teamname}\nTeams: {all_teams}"
                )  # error
        else:
            # all teams
            buff = ""
    ...
    async def blocked(self, ctx: discord.ApplicationContext):
        ...
        if team:
            ...
            await ctx.respond(
                f"Expected team {BLOCKED_TEAM_NAME} not configured"
            )  # error

    # ticket 484 - http://10.10.0.218/issues/484
    # block users based on name (not discord membership)
    @scn.command(
        description="block specific a email address and reject all related tickets"
    )
    async def block(self, ctx: discord.ApplicationContext, username: str):
        ...
        if user:
            ...
            await ctx.respond(
                f"Blocked user: {user.login} and rejected all created tickets"
            )
        else:
    ...
    async def approve(self, ctx: discord.ApplicationContext):
        if self.is_admin(ctx.user):
            ...
            if len(users) > 0:
                await ctx.respond(
                    "Approve Registered Users", view=ApproveUserView(self.bot, users)
                )
            else:
    ...
    #     if self.bot.is_pii_admin(ctx.user): #TODO CHANGE LATER
    #         await ctx.defer(ephemeral=True)  # TODO CHANGE LATER
    #         ticket = self.redmine.ticket_mgr.get(ticket_id)
    #         if ticket:
    #             # ticket = self.redmine.unredact_ticket(ticket) TODO CHANGE LATER
    #             #await self.bot.formatter.print_ticket(ticket, ctx)

    #             # await ctx.respond(embed=self.formatter.ticket_embed(ctx, ticket), ephemeral=True)
    #             await ctx.followup.send(embed=self.formatter.ticket_embed(ctx, ticket), ephemeral=True)  # Changed to followup TODO CHANGE LATER

    #         else:
    #             # TODO user error
    #             # await ctx.respond(f"Ticket {ticket_id} not found.")
    #             await ctx.followup.send(f"Ticket {ticket_id} not found.")  # Changed to followup TODO CHANGE LATER
    #     else
    #         ticket = self.redmine.ticket_mgr.get(ticket_id)
    #         await ctx.followup.send(embed=self.formatter.ticket_embed(ctx, ticket), ephemeral=True)
